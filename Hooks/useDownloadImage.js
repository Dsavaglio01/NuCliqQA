...
export const useDownloadImage = ({ fileName, data, backendURL, dataForURL, setOgUserStoryModal }) => {
  ...
  const addImage = async () => {
    try {
      ...
      const response = await fetch(data[0].post);
      const blob = await response.blob();
      const storageRef = ref(storage, fileName);

      await uploadBytesResumable(storageRef, blob);
      await getLink(fileName, data[0]);
    } catch (error) {
  ...
  return { addImage, downloadLoading };
};
