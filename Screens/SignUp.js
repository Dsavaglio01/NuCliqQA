...
import { getAuth, createUserWithEmailAndPassword, sendSignInLinkToEmail, signInWithEmailLink, fetchSignInMethodsForEmail} from 'firebase/auth';
...
const SignUp = () => {
  ...
  try {
    await SafariView.isAvailable(); // Check if Safari View Controller is available
    await SafariView.show({
      url: 'https://nucliq-c6d24.firebaseapp.com/__/auth/action',
    });

    // ... Handle redirect after authentication
  } catch (error) {
    // ... Handle error if Safari View Controller isn't available
  }
  ...
  return (
    <ImageBackground style={styles.signUpcontainer} source={require('../assets/background2.jpg')} resizeMode="cover">
        ...
        <View style={[styles.main, {backgroundColor: theme.backgroundColor}]}>
          <RegisterHeader onPress={() => navigation.navigate('FirstLogin')} colorOne={{borderColor: '#3286ac'}}/>
            <View style={{marginLeft: '5%', marginRight: '5%', marginTop: '10%'}}>
                <InputBox text={email.length > 0 ? email : 'Email Address'} icon={'email-outline'} password={false} value={email} onChangeText={setEmail}/>
                ...
                <Text style={[styles.agreed, {color: theme.theme != 'light' ? "#9EDAFF" : "#005278"}]}>By continuing, you agree to our <Text style={{textDecorationLine:'underline'}} onPress={() => navigation.navigate('TandC')}>
                ...
                <View style={{marginHorizontal: '5%'}}>
                    {!success ? 
                    <NextButton text={'Verify Email to Create an Account'} onPress={email.length == 0 ? () => Alert.alert('No email', 'Valid Email must be present to create an account', [
      {
        text: 'Cancel',
        onPress: () => console.log('Cancel Pressed'),
        style: 'cancel',
      ...
      {text: 'OK', onPress: () => console.log('OK Pressed')},
    ]) : /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) ? () => handleSignUp(email) : () => Alert.alert('Invalid Email', 'Please enter a valid email address')}/> : 
    <Text style={{fontSize: 15.36, fontFamily: 'Montserrat_500Medium', color: theme.color}}>An email has been sent to {email}. Click on the link provided to create an account!</Text>}
                </View>
            </View>
        </View>
        </TouchableWithoutFeedback>
