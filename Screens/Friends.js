  ...
  useEffect(() => {
    const getData = async() => {
      ...
      unsub = onSnapshot(doc(db, 'profiles', user.uid), (e) => {
        //setSearchKeywords(e.data().searchKeywords)
    ...
    if (objectIndex !== -1) {
      ...
      setFriendsInfo(updatedData);
                ...
                {text: 'OK', onPress: async() => {
                  try {
                    console.log(newFriend, friendId, user.uid)
  ...
  async function addFriend(item) {
   ...
   // let url = 'https://us-central1-nucliq-c6d24.cloudfunctions.net/addFriendTwo'
    ...
    const response = await fetch(`${BACKEND_URL}/api/addFriendTwo`, {
      ...
      body: JSON.stringify({ data: {item: item, newFriend: newFriend, user: user.uid, username: ogUsername, smallKeywords: smallKeywords, largeKeywords: largeKeywords}}), // Send data as needed
    })
    const data = await response.json();
    //console.log(data)
      if (data.request) {
