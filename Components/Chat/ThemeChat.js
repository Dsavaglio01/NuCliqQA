        ...
        <View style={item.user == user.uid ? styles.user: styles.notUser}>
            ...
            />
              ...
              <Animated.View style={item.user == user.uid ? userBubbleStyle : bubbleStyle}>
                <Text allowFontScaling={false} style={item.user == user.uid ? [styles.postUsername, {color: "#121212"}] 
                    : [styles.postUsername, {color: "#fafafa"}]}>Theme unavailable</Text>
            ...
            <View style={item.user == user.uid ? styles.user : styles.notUser}>
                ...
                <Animated.View style={item.user == user.uid ? userPostContainer : postContainer}>
                    <TouchableOpacity onPress={() => handleThemePress(item)}  activeOpacity={1} onLongPress={() => toggleSave(true, item, newMessages, updateNewMessages)}>
                        <View style={styles.headerContainer}>
                            <View style={styles.header}>
                                <Text allowFontScaling={false} numberOfLines={1} style={item.user == user.uid ? [styles.postUsername, {color: "#121212"}] 
                                    : [styles.postUsername, {color: "#fafafa"}]}>Theme: {item.message.theme.name}</Text>
                            </View>
                        </View>
                        <View style={{marginTop: '5%'}}>
                        {item.message.theme.images[0] ? 
                            <FastImage source={{uri: item.message.theme.images[0]}} style={[styles.image, {width: 220}]}/> :
                            <FastImage source={require('../../assets/defaultpfp.jpg')} style={styles.image}/>
                        }
                        </View>
                    </TouchableOpacity>
                    <View style={[styles.timestampContainer, item.user != user.uid && styles.userTimestampContainer]}>
                        {getDateAndTime(item.timestamp) ? <Text style={item.user == user.uid ? styles.userTimestamp 
                            : styles.timestamp}>{getDateAndTime(item.timestamp)}</Text> : null}
                    </View>
